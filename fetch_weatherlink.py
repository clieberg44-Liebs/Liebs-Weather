...
def discover_station_id():
    ...
    print(f"Auto-detected station_id={STATION_ID}")
    return STATION_ID
...
def clicks_to_inches(clicks, rain_collector=1):
    if clicks is None:
        return None
...
def aggregate_day(records, day_iso):
    highs, lows, rains = [], [], []
    for r in records:
...
def run_refresh(station_id):
    ...
    # DEBUG output to diagnose field-name mismatches
    print(f"DEBUG top-level keys: {list(cur_json.keys())}")
    sensors = cur_json.get("sensors", [])
    print(f"DEBUG sensor count: {len(sensors)}")
    for idx, sensor in enumerate(sensors):
        stype = sensor.get("sensor_type")
        dstype = sensor.get("data_structure_type")
        records = sensor.get("data", [])
        print(f"DEBUG sensor {idx}: type={stype} dstype={dstype} records={len(records)}")
        if records:
            sample_keys = sorted(list(records[0].keys()))
            print(f"DEBUG sensor {idx} fields: {sample_keys}")

    current = extract_current(cur_json)
    print(f"DEBUG extracted: {current}")
    current_ts = cur_json.get("generated_at")

    print("Fetching today's archive...")
    today_rec = fetch_day(station_id, today)

    daily_current = existing.get("daily2026", [])
    if today_rec:
    ...
    last_fetch_dt = datetime.fromtimestamp(current_ts, TZ) if current_ts else now
    export["lastFetch"] = last_fetch_dt.isoformat()
...
def main():
    ...
    if args.mode == "refresh":
        ...
        raise SystemExit("Backfill disabled in debug build - use refresh mode")
